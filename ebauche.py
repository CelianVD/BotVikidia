# Script parourant les pages créées dans les dernières 24 heures et ajoute le bandeau ébauche si besoin, par Célian et moi.Utilisé par BotCélian et LuffyBot
...
def has_travaux(text):
    return re.search(r'\{\{\s*travaux\s*\|[^}]+?\}\}', text, re.IGNORECASE)
...
def main():
    ...
    for change in recent_newpages:
        ...
        if has_travaux(text):
            log(f"{page.title()} ignorée : modèle {{Travaux}} détecté")
            continue
